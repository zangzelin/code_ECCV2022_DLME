...
class MyLoss(nn.Module):
    ...
    def _TwowaydivergenceLoss(self, P_, Q_):
        ...
        losssum1 = (P_ * torch.log(Q_ + EPS))[select_index_near].sum()
        losssum2 = ((1-P_) * torch.log(1-Q_ + EPS))[select_index_far].sum()
        losssum = -1*(losssum1 + losssum2) / (select_index_near.sum()+select_index_far.sum())
