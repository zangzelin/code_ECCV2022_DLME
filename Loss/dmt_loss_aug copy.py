...
class MyLoss(nn.Module):
    ...
    def forward(self, input_data, input_data_aug, latent_data, latent_data_aug, rho, sigma, v_latent, ):
        ...
        distlatent = self._DistanceSquared(latent)
        
        
        P = self._Similarity(
                dist=disInput,
                rho=torch.cat([rho, rho]),
                sigma_array=torch.cat([sigma, sigma]),
    ...
    def _TwowaydivergenceLoss(self, P_, Q_):
        ...
        EPS = 1e-12
        # select_index_near = (P_ > Q_) & (P_ > self.near_bound)
